...
class JourneyService {
    ...
    async acceptJourney(journeyId, driverId) {
        ...
        // Validate driver, fallback to any registered driver if driverId is missing or testing
        let driver = null;
        if (driverId) {
            driver = await Driver.findById(driverId).populate('userId');
        }
        if (!driver) {
            driver = await Driver.findOne({}).populate('userId');
        }

        if (!driver) {
            throw new Error('No driver profiles found in database. Please register a driver profile first.');
        }

        // Ensure driver status allows acceptance
        if (!driver.status.isOnline) {
            driver.status.isOnline = true;
        }
        if (!driver.status.isVerified) {
            driver.status.isVerified = true;
        }
        await driver.save();
    ...
    async updateJourneyStatus(journeyId, driverId, newStatus) {
        ...
        } else if (newStatus === 'STARTED' || newStatus === 'ON_THE_WAY') {
    ...
    async completeJourney(journeyId, driverId, completionData) {
        ...
        // Validate journey is started or on the way
        if (journey.status !== 'STARTED' && journey.status !== 'ON_THE_WAY') {
    ...
    async getJourneyById(journeyId, userId) {
        ...
        // Validate user is part of this journey (or any user if status is REQUESTED)
        const userObjId = (userId?._id || userId)?.toString();
        const riderObjId = (journey.riderId?._id || journey.riderId)?.toString();
        const isRider = Boolean(riderObjId && userObjId && riderObjId === userObjId);

        const driverDocId = (journey.driverId?._id || journey.driverId)?.toString();
        const driverUserId = (journey.driverId?.userId?._id || journey.driverId?.userId)?.toString();
        const isDriver = Boolean(
            userObjId && (
                (driverDocId && userObjId === driverDocId) ||
                (driverUserId && userObjId === driverUserId)
            )
        );
        const isRequested = journey.status === 'REQUESTED';

        if (!isRider && !isDriver && !isRequested) {
    ...
    // ============================================
    // GET AVAILABLE JOURNEYS (DRIVER)
    // ============================================
    // Purpose: Get all REQUESTED journeys for drivers to accept
    //
    // Returns: Array of requested journeys
    async getAvailableJourneys() {
        const journeys = await Ride.find({ status: 'REQUESTED' })
            .populate('riderId', 'name email phone')
            .sort({ createdAt: -1 });
    ...
    formatJourneyResponse(journey) {
        const riderObj = journey.riderId ? {
            _id: journey.riderId._id || journey.riderId,
            name: journey.riderId.name || 'Rider',
            email: journey.riderId.email || '',
            phone: journey.riderId.phone || ''
        } : { _id: 'rider', name: 'Rider', email: '', phone: '' };

        const driverObj = journey.driverId ? {
            _id: journey.driverId._id || journey.driverId,
            name: journey.driverId.userId?.name || 'Driver Partner',
            email: journey.driverId.userId?.email || '',
            phone: journey.driverId.userId?.phone || '+91 9876543210',
            vehicleModel: journey.driverId.vehicleInfo?.vehicleModel || 'Maruti Swift Dzire',
            vehicleColor: journey.driverId.vehicleInfo?.vehicleColor || 'White',
            vehicleNumber: journey.driverId.vehicleInfo?.vehicleNumber || 'DL01CA1234',
            rating: journey.driverId.stats?.rating || 4.9,
            userId: journey.driverId.userId ? {
                _id: journey.driverId.userId._id || journey.driverId.userId,
                name: journey.driverId.userId.name || 'Driver Partner',
                email: journey.driverId.userId.email || '',
                phone: journey.driverId.userId.phone || '+91 9876543210'
            } : null
